...
public class RestClient {
	...
	protected RestClient(String serverURI, String servicePath ) {
        ...
        try {
            ...
            Log.severe("Erro ao obter SSLContext: " + e.getMessage());
            throw new RuntimeException(e);
        }

        ClientRequestFilter secretFilter = ctx -> {
            var secret = System.getProperty("secret", "");
            if (!secret.isEmpty() && ctx.getUri().getPath().contains("/admin"))
                ctx.getHeaders().add(SECRET_HEADER, secret);
        };
        this.client = ClientBuilder.newBuilder().sslContext(sslContext).withConfig(config).register(secretFilter).build();
    ...
    protected <T> Result<T> singleTry(Supplier<Result<T>> func) {
        try {
            return func.get();
        } catch (ProcessingException x) {
            return error(TIMEOUT);
