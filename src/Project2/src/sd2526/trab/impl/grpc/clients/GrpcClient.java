...
public class GrpcClient {
	...
	protected GrpcClient(String serverUrl) {
        ...
        try {
            ...
            tempChannel = NettyChannelBuilder.forAddress(serverURI.getHost(), serverURI.getPort()).sslContext(context).enableRetry().build();
        } catch (Exception e) {
            System.err.println("Failed to initialize gRPC SSL context for client: " + e.getMessage());
            e.printStackTrace();
