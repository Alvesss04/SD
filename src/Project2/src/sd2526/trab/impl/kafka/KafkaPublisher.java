...
public class KafkaPublisher {
    ...
    public long publish(String topic, String key, String value) {
        try {
            Future<RecordMetadata> promise = producer.send(new ProducerRecord<>(topic, key, value));
    ...
    public long publish(String topic, String value) {
        try {
            Future<RecordMetadata> promise = producer.send(new ProducerRecord<>(topic, value));
            RecordMetadata rec = promise.get();
            return rec.offset();
        } catch (ExecutionException | InterruptedException x) {
            x.printStackTrace();
